...
/* Bare-metal override: delegate to bare_sdvbs_common.h in pmu_sdvb/ */
#ifndef _SDVBS_COMMON_
#define _SDVBS_COMMON_
#ifdef BARE_METAL
#include "bare_sdvbs_common.h"
#else
#define _SDVBS_COMMON_
...
static inline void ff_printf(const char *pcFormat, ...) {
    ...
    va_end(args);
    UART_polled_tx_string(gp_my_uart, (uint8_t *)buffer);
}

int my_scanf(FF_FILE *stream, const char *format, void *result);
